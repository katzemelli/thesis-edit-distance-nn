...
Prof: *"aa freq plot muss relativ sortiert sein, d.h. je alphabet von häufigstem zu seltestem …
hier könnten theoretisch auch alle vier zusammen rein. Das gleiche gilt für die übergangstabellen
damit sie etwas vergleichbarer sind."*
- **Letter-frequency plot → per-alphabet RANK-sorted** (x-axis = frequency rank within the alphabet,
  1 = most frequent; not the absolute letter), all four sets overlaid:
  `outputs/folie_revisions/C_folie9_letterfreq_ranksorted.png`. Now the *shapes* are comparable:
  synthetic flat at 1/20, AA gentle decay, 3Di steeper, SS a 3-letter cliff.
- **Transition heatmaps → rows/cols RANK-sorted by each alphabet's own frequency**, four panels,
  shared colour scale with a "chance (0.05)" marker:
  `outputs/folie_revisions/D_folie9_transition_ranksorted.png`. Comparable now: synthetic/AA pale
  (no structure), 3Di real structure, SS collapses onto its 3-letter self-transition block.
- **Score distribution → add the synthetic (training) curve** so all four are overlaid:
  `outputs/folie_revisions/B_folie9_score_density_four_sets.png` (synthetic dashed/orange spans the
  full range and is the only set with real mass ≥0.70 — the transfer point).
